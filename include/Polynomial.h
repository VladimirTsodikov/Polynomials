...
bool Polynomial::operator<(const Polynomial& Pol) const
{
	...
	bool lower = true, reached_the_end=true;			//если попали сюда, полиномы уже точно не равны
	Monomial* tmp1 = head->next, * tmp2 = Pol.head->next;
	while (tmp1 != head && tmp2 != Pol.head)	//на tmp2 условие уже нужно, т.к. количество звеньев в полиномах может быть различно
	{
		...
		else if (*tmp1 > *tmp2)	{ lower = false; break;	}	//выявили, что наш полином больше, не дойдя до конца (раньше, чем закончился общий промежуток)
		else if (*tmp1 < *tmp2) { reached_the_end = false; break; } //выявили, что наш полином меньше, не дойдя до конца (раньше, чем закончился общий промежуток)	
	}
	if (lower == true && reached_the_end == true)	//если дошли до конца общего размера двух полиномов, и все звенья были равны, то больше будет тот полином, у которого больше звеньев. Полиномы не могут быть целиком равны, т.к. первая строчка пройдена
